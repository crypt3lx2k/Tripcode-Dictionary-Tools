...
class TripcodeParser (OfflineParser):
    """
    Adds handling for tripcode databases to the OfflineParser.
    """
    def __init__ (self, *args, **kwargs):
        super(TripcodeParser, self).__init__(*args, **kwargs)

        self.add_argument (
            '--public-file',
            metavar='file', type=str, default=defaults['public_file'],
            help='database for tripcodes, defaults to {public_file}'.format (
                **defaults
            )
        )
        ...
        self.add_argument (
            '--secure-file',
            metavar='file', type=str, default=defaults['secure_file'],
            help='database for tripcodes, defaults to {secure_file}'.format (
                **defaults
            )
        )
